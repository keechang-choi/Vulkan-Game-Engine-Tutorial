...
glm::vec2 GravityPhysicsSystem::computeForce(lve::LveGameObject& fromObj,
...
void Vec2FieldSystem ::update(const GravityPhysicsSystem& physicsSystem,
  ...
  // For each field line we caluclate the net graviation force for that point in
  // space
  for (auto& vf : vectorField) {
    ...
    // values were chosen just through trial and error based on what i liked the
    // look of and then the field line is rotated to point in the direction of
    // the field
